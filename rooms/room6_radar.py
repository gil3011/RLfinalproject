...
only a fan of `K` radar rays reporting the nearest obstacle within range `X`. Obstacle
layouts are re-sampled EVERY episode, so the headline metric is GENERALISATION: escape
rate on held-out layouts the training never saw.
...
depends on the radar readings (the layout), so a 2-D max_a Q(x,y) slice would lie. The
honest visuals are the live radar and the trajectory heatmap.
"""
...
LEGEND = ("🤖 Start · 🏁 Exit · ⬤ Obstacles (fatal on contact) · "
          "radar rays turn 🔴 when they detect an obstacle within range X")
...
# ───────────────────────── board figure ─────────────────────────
def _board_figure(obstacles, agent=None, vel=None, radar=None, ray_dirs=None,
                  radar_range=3.0, path=None, dead=False, heat=None, countdown=None):
    fig = go.Figure()
    ...
    # live radar rays from the agent
    if agent is not None and radar is not None and ray_dirs is not None:
        for k, u in enumerate(ray_dirs):
            hit = radar[k] < radar_range - 1e-6
            reach = radar[k] if hit else radar_range
            x1, y1 = agent[0] + u[0] * reach, agent[1] + u[1] * reach
            fig.add_trace(go.Scatter(
                x=[agent[0], x1], y=[agent[1], y1], mode="lines",
                line=dict(color=("rgba(231,76,60,0.95)" if hit else "rgba(180,200,230,0.35)"),
...
def _env_controls():
    ...
    st.caption("The agent sees only its radar — not obstacle coordinates. Layouts "
               "are regenerated every episode, so it must learn to *react*, not memorise.")
    c1, c2 = st.columns(2)
    radar_range = c1.slider("⭐ Agent radar range X (m)", 1.0, 10.0, 3.0, 0.5,
        help="How far each ray sees. Distance is measured CENTER-TO-CENTER (agent centre "
             "to obstacle centre), so usable clearance is X − 0.5 m: 'detected at X' is not "
             "'collides at X'. More range = more warning.")
    n_rays = c2.select_slider("Radar rays K", [4, 8, 16, 32], 8,
        help="Number of rays fanned 360° around the agent. More rays = finer perception "
             "(the biggest lever on avoidance) but a larger observation to learn.")
    c3, c4 = st.columns(2)
    n_obstacles = c3.slider("Obstacle count", 2, 12, 6, 1,
        help="Discs (0.5 m each) placed randomly every episode, always leaving a "
             "traversable path. A reactive controller escapes ~53% at 6, ~26% at 12.")
    obstacle_speed = c4.slider("Dynamic-obstacle speed (m/s)", 0.0, 1.5, 0.0, 0.1,
        help="0 = static (default). Above 0, obstacles drift and bounce — the "
             "observation switches on a per-ray range-rate channel so the network can "
             "tell approaching from receding (a single sweep would be non-Markov).")
    ...
    return dict(radar_range=radar_range, n_rays=n_rays, n_obstacles=n_obstacles,
...
def _algo_row():
    ...
    st.caption("Same recipe as Room 5 — Double DQN + experience replay + reward "
               "scaling — now over a partially-observed radar state.")
    ...
    n_episodes = c1.slider("Training episodes", 500, 5000, 1200, 100,
        help="A larger, partially-observed input needs more episodes than Room 5. "
             "Training shows a live progress bar (roughly a few minutes; ~70% escape "
             "on unseen layouts by 1,200 at the defaults).")
...
# ───────────────────────── render ─────────────────────────
def render():
    st.markdown("### Room 6 · Advanced DQL — Radar & Obstacles")
    st.caption("Cross a chamber full of obstacles you can only sense through radar. "
    ...
    with st.expander("ℹ️ About this room", expanded=False):
        st.markdown(
            ...
            "where the obstacles are, only `K` radar distances.\n\n"
            ...
            "memorised path.\n"
            "* **Radar is center-to-center.** A ray reports the distance to an obstacle's "
            "*centre*, capped at range X — so it detects sooner than it collides.\n"
            ...
            "* **No value-field heatmap** here: value depends on the radar (the layout), so "
            "a flat Q(x, y) slice would mislead. Watch the **live radar** and the "
    ...
    with board_col:
        board = st.empty()
        st.caption(LEGEND)
        prev_env = RadarArena(n_obstacles=env["n_obstacles"], n_rays=env["n_rays"],
                              radar_range=env["radar_range"], friction=env["friction"],
        ...
        board.plotly_chart(
            _board_figure(prev_info["obstacles"], agent=prev_info["agent"],
                          radar=prev_info["radar"], ray_dirs=prev_env.ray_dirs,
                          radar_range=env["radar_range"]),
    ...
    def make_train_env():
        return RadarArena(n_obstacles=env["n_obstacles"], n_rays=env["n_rays"],
                          radar_range=env["radar_range"], friction=env["friction"],
    ...
    sig = (env["radar_range"], env["n_rays"], env["n_obstacles"], env["obstacle_speed"],
    ...
    view_env = RadarArena(n_obstacles=env["n_obstacles"], n_rays=env["n_rays"],
                          radar_range=env["radar_range"], friction=env["friction"],
                          obstacle_speed=env["obstacle_speed"], max_steps=150)
    ...
    if show_heat:
        ...
        if cp_i not in cache:
            _, _, _, paths = evaluate_policy(
                ...
                lambda: RadarArena(n_obstacles=env["n_obstacles"], n_rays=env["n_rays"],
                                   radar_range=env["radar_range"], friction=env["friction"],
                                   obstacle_speed=env["obstacle_speed"], max_steps=150),
    ...
    if play or gen_room:
        # Both play a single episode on a FRESH unseen layout (ephemeral).
        play_env = RadarArena(n_obstacles=env["n_obstacles"], n_rays=env["n_rays"],
                              radar_range=env["radar_range"], friction=env["friction"],
                              obstacle_speed=env["obstacle_speed"], max_steps=150)
        pr = greedy_rollout(net, play_env, seed=np.random.randint(_GEN_SEED0, 2**31 - 1))
        frames = pr["frames"]
        rd = play_env.ray_dirs
        for c in (3, 2, 1):
            results_board.plotly_chart(
                _board_figure(frames[0]["obstacles"], agent=frames[0]["agent"],
                              radar=frames[0]["radar"], ray_dirs=rd,
                              radar_range=env["radar_range"], heat=heat, countdown=c),
        ...
        for kf in range(len(frames)):
            ...
            results_board.plotly_chart(
                _board_figure(frames[kf]["obstacles"], agent=frames[kf]["agent"],
                              radar=frames[kf]["radar"], ray_dirs=rd,
                              radar_range=env["radar_range"], path=trail,
        ...
        results_board.plotly_chart(
            _board_figure(last["obstacles"], agent=last["agent"], radar=last["radar"],
                          ray_dirs=view_env.ray_dirs, radar_range=env["radar_range"],
